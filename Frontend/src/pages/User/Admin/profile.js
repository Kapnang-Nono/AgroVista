...
import { MSG_VAR, baseURL, admin_ID } from '../../../constants';
...
const AdminProfile = () => {
  ...
  useEffect(() => {
    try {
       fetch(`${baseURL}/api/global/account-details?user=${admin_ID}`)
  ...
  const Updateprofile = async () => {
    ...
    try {
      const response = await fetch(`${baseURL}/api/global/update-picture/${admin_ID}`, {
        method:'PATCH',
        body: formdata
      })
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${baseURL}/api/farmer/update-profile/${admin_ID}`, {
        method: 'PUT',
