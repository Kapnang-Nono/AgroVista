...
import { MSG_VAR, baseURL, driver_ID } from '../../../constants';
...
const DriverProfile = () => {
  ...
  useEffect(() => {
    try {
       fetch(`${baseURL}/api/global/account-details?user=${driver_ID}`)
  ...
  }, [location.pathname === "/driverdashboard/profile"])
  ...
  const Updateprofile = async () => {
    ...
    try {
      const response = await fetch(`${baseURL}/api/global/update-picture/${driver_ID}`, {
        method:'PATCH',
        body: formdata
      })
      ...
      if(resp.status === MSG_VAR.ERROR){
        ...
        localStorage.setItem('driver-picture', resp.picture)
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${baseURL}/api/farmer/update-profile/${driver_ID}`, {
        method: 'PUT',
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="mb- mt-2" >
        ...
        <div className="flex items-center">
          ...
          >
            {selectedpicture ? (
              <img src={`${baseURL}/profile/${selectedpicture}`} alt="Profile" className="w-full h-full object-cover" />
            ) : (
