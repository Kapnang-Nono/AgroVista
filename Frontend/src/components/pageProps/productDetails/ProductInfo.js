...
const ProductInfo = ({ productInfo, productCategory }) => {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <button
        ...
        className="w-full py-4 bg-green-500 hover:bg-green-600 duration-300 text-white text-lg font-titleFont rounded-sm"
      >
        Add to Cart
      </button>
      <p className="font-normal text-sm">
        <span className="text-base font-medium"> Categories:</span> 
        {productCategory.map((el) => <span>{el.catName + ' '}</span>)}
      </p>
    </div>
  );
};
