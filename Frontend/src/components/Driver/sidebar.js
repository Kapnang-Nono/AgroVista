...
import { Home, Package, Users, Truck, LogOut, Menu, User, Book } from 'lucide-react';
      ...
      >
        ...
        <nav>
          <ul className="space-y-2 mt-5">
            {[
              { name: 'Dashboard', icon: Home, path: '/driverdashboard' },
              { name: 'Jobs', icon: Truck, path: '/driverdashboard/jobs' },
              // { name: 'History', icon: Book, path: '/dashboard/driverhistory' },
              { name: 'Profile', icon: User, path: '/driverdashboard/profile' },
            ].map((item) => (
              <li key={item.name}>
                <NavLink
                  to={item.path}
